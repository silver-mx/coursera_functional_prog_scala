...
class FunSetSuite extends FunSuite {
  ...
  test("intersection contains all elements that exist in each set") {
    new TestSets {
      val s = union(s1, s2)
      val x = union(s1, s3)
      val z = intersect(s, x)
  ...
  test("difference contains all elements that exist in A but not in B") {
    new TestSets {
      val s = union(s1, s2)
      val x = union(s1, s3)
      val z = diff(s, x)
  ...
  test("map") {
    new TestSets {
      ...
      //printSet(z)
      assert(contains(z, 0), "Map 0")
      assert(!contains(z, 1), "Map 1")
